...
import InventarioDetailModal from 'components/admin/inventario/InventarioDetailModal';
...
const InventarioContent = () => {
  ...
  const [subclaseFilter, setSubclaseFilter] = useState('');
  const [estadoFilter, setEstadoFilter] = useState('');
  const [criticoFilter, setCriticoFilter] = useState('');
  ...
  const [editingEquipo, setEditingEquipo] = useState<InventarioEquipo | null>(null);
  const [viewingEquipo, setViewingEquipo] = useState<InventarioEquipo | null>(null);
  const [syncing, setSyncing] = useState(false);

  const [page, setPage] = useState(0);
  const pageSize = 25;

  const buildFilters = useCallback((): InventarioEquipoFilters => {
    const filters: InventarioEquipoFilters = {};
    if (servicioFilter) filters.servicio = servicioFilter;
    if (claseFilter) filters.clase = claseFilter;
    if (subclaseFilter) filters.subclase = subclaseFilter;
    if (estadoFilter) filters.estado = estadoFilter;
    if (criticoFilter) filters.criticoApoyo = criticoFilter;
    ...
    return filters;
  }, [servicioFilter, claseFilter, subclaseFilter, estadoFilter, criticoFilter, convenioFilter, cumplimientoFilter, searchQuery]);
  ...
  const fetchEquipos = useCallback(async () => {
    ...
    try {
      ...
      const result = await InventarioEquipoService.getInventario(filters);
  ...
  const fetchStatsGlobal = useCallback(async () => {
    try {
      const s = await InventarioEquipoService.getEstadisticasFisicas();
      setStatsGlobal(s);
    } catch (error) {
  ...
  const handleView = (equipo: InventarioEquipo) => {
    setViewingEquipo(equipo);
    setDetailModalOpen(true);
  };

  const handleEdit = (equipo: InventarioEquipo) => {
  ...
  const handleDelete = async (equipo: InventarioEquipo) => {
    ...
    if (result.isConfirmed) {
      try {
        await InventarioEquipoService.delete(equipo.id);
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const filters = buildFilters();
      const data = await InventarioEquipoService.exportar(filters);
      ...
      const wsData = data.map((e) => ({
        ...
        Valor: e.valor,
        'Fecha Adquisicion': e.fechaAdquisicion,
        'Vida Util (anios)': e.vidaUtil,
        Estado: ESTADO_LABELS[e.estado] || e.estado,
        'Critico/Apoyo': CRITICO_LABELS[e.criticoApoyo] || e.criticoApoyo,
      ...
      XLSX.writeFile(wb, 'inventario_equipos_medicos.xlsx');
    } catch (error: any) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const headers = [
        ...
        'estado', 'criticoApoyo', 'frecuencia', 'garantiaInicio', 'garantiaFinal', 'fechaBaja',
      ];
      const exampleRows = [
        {
          servicio: 'Neonatologia',
          clase: 'Apoyo Diagnostico',
          subclase: 'Alto Costo',
          nombreEquipo: 'ECOGRAFO',
          marca: 'GE Healthcare',
          modelo: 'LOGIQ E10',
          serie: 'SN-12345',
          inventario: 'INV-001',
          ...
          criticoApoyo: 'C',
          frecuencia: 6,
          garantiaInicio: '2023-01-15',
          garantiaFinal: '2025-01-15',
          fechaBaja: '',
        },
        {
          servicio: 'Urgencia',
          clase: 'Monitoreo',
          subclase: 'Mediano Costo',
          nombreEquipo: 'MONITOR SIGNOS VITALES',
          marca: 'Philips',
          modelo: 'MX800',
          serie: 'SN-67890',
          inventario: 'INV-002',
      ...
      XLSX.writeFile(wb, 'formato_inventario_equipos.xlsx');
    } catch (error: any) {
    ...
    input.onchange = async (e: any) => {
      ...
      try {
        ...
        const items = rows.map((row) => ({
          ...
          criticoApoyo: String(row.criticoApoyo || 'A'),
        ...
        const confirm = await Swal.fire({
          ...
          text: `Se importaran ${items.length} equipos. Desea continuar?`,
          icon: 'question',
          showCancelButton: true,
          cancelButtonText: 'Cancelar',
        ...
        const result = await InventarioEquipoService.importar(items);
        Swal.fire(
  ...
  return (
    <div className="flex w-full flex-col gap-5">
      ...
      <div className="flex flex-col gap-3 pt-5 sm:flex-row sm:items-center sm:justify-between lg:pt-10">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Gestione el inventario de equipos medicos, su estado y mantenimiento.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <Card extra="p-4">
          <p className="text-sm text-gray-500 dark:text-gray-400">Activos (Bueno)</p>
      ...
      <Card extra="p-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <select
            ...
            onChange={(e) => { setServicioFilter(e.target.value); setPage(0); }}
          ...
          <select
            value={claseFilter}
            onChange={(e) => { setClaseFilter(e.target.value); setPage(0); }}
            className="h-10 rounded-lg border border-gray-200 bg-white px-3 text-sm outline-none dark:border-navy-600 dark:bg-navy-700 dark:text-white"
          >
          ...
          <select
            value={subclaseFilter}
            onChange={(e) => { setSubclaseFilter(e.target.value); setPage(0); }}
            className="h-10 rounded-lg border border-gray-200 bg-white px-3 text-sm outline-none dark:border-navy-600 dark:bg-navy-700 dark:text-white"
          >
          ...
          <select
            value={criticoFilter}
            onChange={(e) => { setCriticoFilter(e.target.value); setPage(0); }}
            className="h-10 rounded-lg border border-gray-200 bg-white px-3 text-sm outline-none dark:border-navy-600 dark:bg-navy-700 dark:text-white"
          >
            <option value="">Critico/Apoyo</option>
          ...
          <div className="flex h-10 items-center rounded-lg border border-gray-200 bg-white px-3 dark:border-navy-600 dark:bg-navy-700 sm:col-span-2">
              ...
              {equipos.map((eq) => (
                ...
                >
                  ...
                  <td className="hidden px-3 py-3 md:table-cell">
                    <span className="text-sm text-navy-700 dark:text-gray-300">{eq.servicio}</span>
                  </td>
                  <td className="hidden px-3 py-3 lg:table-cell">
                    <span className="text-sm text-navy-700 dark:text-gray-300">{eq.clase}</span>
                  </td>
                  <td className="hidden px-3 py-3 md:table-cell">
                  ...
                  <td className="hidden px-3 py-3 lg:table-cell">
                    <span className="text-sm text-navy-700 dark:text-gray-300">{eq.serie}</span>
                  </td>
                  ...
                  <td className="hidden px-3 py-3 text-center sm:table-cell">
                    <span
                      className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${
                        CRITICO_COLORS[eq.criticoApoyo] || 'bg-gray-100 text-gray-600'
                      }`}
                    >
                      {CRITICO_LABELS[eq.criticoApoyo] || eq.criticoApoyo}
                    </span>
                  </td>
