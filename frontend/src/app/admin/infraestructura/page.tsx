...
import InfraestructuraFormModal from 'components/admin/infraestructura/InfraestructuraFormModal';
import InfraestructuraDetailModal from 'components/admin/infraestructura/InfraestructuraDetailModal';
...
const InfraestructuraContent = () => {
  const currentUser = Parse.User.current();
  const userAccessLevel = currentUser?.get('accessLevel') || 1;

  const [componentes, setComponentes] = useState<InventarioInfraestructura[]>([]);
  ...
  const [editingComponente, setEditingComponente] = useState<InventarioInfraestructura | null>(null);
  const [viewingComponente, setViewingComponente] = useState<InventarioInfraestructura | null>(null);
  const [syncing, setSyncing] = useState(false);

  const [page, setPage] = useState(0);
  const pageSize = 25;

  const buildFilters = useCallback((): InventarioInfraestructuraFilters => {
    const filters: InventarioInfraestructuraFilters = {};
    if (sistemaFilter) filters.sistema = sistemaFilter;
    ...
    return filters;
  }, [sistemaFilter, ubicacionFilter, estadoFilter, criticidadFilter, convenioFilter, cumplimientoFilter, searchQuery]);
  ...
  const fetchComponentes = useCallback(async () => {
    ...
    try {
      ...
      const result = await InventarioInfraestructuraService.getInventario(filters);
      setComponentes(result.results);
      setTotal(result.total);
    } catch (error) {
      console.error('Error cargando inventario infraestructura:', error);
    } finally {
      setLoading(false);
    }
  }, [buildFilters, page]);
  ...
  const fetchStatsGlobal = useCallback(async () => {
    try {
      const s = await InventarioInfraestructuraService.getEstadisticasFisicas();
      setStatsGlobal(s);
    } catch (error) {
  ...
  const handleView = (comp: InventarioInfraestructura) => {
    setViewingComponente(comp);
    setDetailModalOpen(true);
  };

  const handleEdit = (comp: InventarioInfraestructura) => {
  ...
  const handleDelete = async (comp: InventarioInfraestructura) => {
    const result = await Swal.fire({
      title: 'Eliminar componente',
      text: `Se eliminara el componente: "${comp.componente}" (Codigo: ${comp.codigoInterno || 'N/A'})`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        await InventarioInfraestructuraService.delete(comp.id);
        Swal.fire('Eliminado', 'El componente ha sido eliminado.', 'success');
        fetchComponentes();
        fetchStatsGlobal();
      } catch (error: any) {
  ...
  const handleSync = async () => {
      ...
      html: `
        <div style="text-align:left">
          ...
          <ul style="margin-top:8px; padding-left:20px">
            ...
            <li><strong>Cumplimiento</strong>: recalcula la fecha del ultimo mantenimiento y los periodos cumplidos de cada componente.</li>
          </ul>
        </div>
      `,
      icon: 'question',
    ...
    try {
      const [conveniosRes, cumplimientoRes] = await Promise.all([
        Parse.Cloud.run('sincronizarConveniosInventario', { inventarioTipo: 'infraestructura' }).catch((e) => ({ _error: e?.message })),
        Parse.Cloud.run('sincronizarCumplimientoMasivo', { dominio: 'infraestructura' }).catch((e) => ({ _error: e?.message })),
      ]);
      ...
      if ((conveniosRes as any)?._error) {
        ...
        if (stats.equiposActualizados > 0) lines.push(`&nbsp;&nbsp;Componentes actualizados: ${stats.equiposActualizados}`);
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const filters = buildFilters();
      const data = await InventarioInfraestructuraService.exportar(filters);
      ...
      const wsData = data.map((e) => ({
        ...
        'Modelo': e.modelo,
        'Serie': e.serie,
        'Codigo Interno': e.codigoInterno,
        'Capacidad': e.capacidad,
        'Fecha Instalacion': e.fechaInstalacion,
        'Vida Util (anios)': e.vidaUtil,
        'Estado': ESTADO_INFRA_LABELS[e.estado] || e.estado,
        'Criticidad': CRITICIDAD_INFRA_LABELS[e.criticidad] || e.criticidad,
        'Frecuencia (meses)': e.frecuencia,
        'Normativa Aplicable': e.normativaAplicable,
        'Ultima Inspeccion': e.fechaUltimaInspeccion,
        'Proxima Inspeccion': e.proximaInspeccion,
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Inventario Infraestructura');
      XLSX.writeFile(wb, 'inventario_infraestructura.xlsx');
    } catch (error: any) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      ...
      const exampleRows = [
        {
          sistema: 'Electrico',
          componente: 'Tablero electrico principal',
          ubicacion: 'Edificio A Piso 1',
          descripcion: 'Tablero general de distribucion electrica del edificio principal',
          marca: 'Schneider Electric',
          modelo: 'Prisma P',
          serie: 'TE-SN-001',
          codigoInterno: 'INF-ELE-001',
          capacidad: '400A / 380V',
          fechaInstalacion: '2018-06-15',
          vidaUtil: 30,
          estado: 'B',
          criticidad: 'Alta',
          frecuencia: 6,
          normativaAplicable: 'INS-1.1, NCh Elec. 4/2003',
          fechaUltimaInspeccion: '2025-10-01',
          proximaInspeccion: '2026-04-01',
          responsable: 'Juan Perez',
          garantiaInicio: '2018-06-15',
          garantiaFinal: '2023-06-15',
          fechaBaja: '',
        },
        {
          sistema: 'Proteccion Incendios',
          componente: 'Red humeda piso 3',
          ubicacion: 'Edificio A Piso 3',
          descripcion: 'Red humeda de proteccion contra incendios, gabinetes y mangueras',
          marca: 'Viking',
          modelo: 'VK-100',
          serie: 'RH-SN-003',
          codigoInterno: 'INF-INC-003',
          capacidad: '100 L/min',
          fechaInstalacion: '2019-03-20',
          vidaUtil: 25,
          estado: 'B',
          criticidad: 'Alta',
          frecuencia: 3,
          normativaAplicable: 'INS-3.1, NCh 2095',
          fechaUltimaInspeccion: '2025-09-15',
          proximaInspeccion: '2025-12-15',
          responsable: 'Maria Gonzalez',
          garantiaInicio: '2019-03-20',
          garantiaFinal: '2024-03-20',
          fechaBaja: '',
        },
      ];
      ...
      XLSX.writeFile(wb, 'formato_inventario_infraestructura.xlsx');
    } catch (error: any) {
    ...
    input.onchange = async (e: any) => {
      ...
      try {
        ...
        const items = rows.map((row) => ({
          ...
          codigoInterno: String(row.codigoInterno || ''),
          ...
          normativaAplicable: String(row.normativaAplicable || ''),
          fechaUltimaInspeccion: String(row.fechaUltimaInspeccion || ''),
          proximaInspeccion: String(row.proximaInspeccion || ''),
        ...
        const confirm = await Swal.fire({
          ...
          text: `Se importaran ${items.length} componentes de infraestructura. Desea continuar?`,
          icon: 'question',
          showCancelButton: true,
          cancelButtonText: 'Cancelar',
        ...
        const result = await InventarioInfraestructuraService.importar(items);
        Swal.fire(
  ...
  return (
    <div className="flex w-full flex-col gap-5">
      ...
      <div className="flex flex-col gap-3 pt-5 sm:flex-row sm:items-center sm:justify-between lg:pt-10">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
            Gestione las instalaciones del establecimiento: sistemas electricos, sanitarios, gases clinicos, proteccion contra incendios y otros.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Card extra="p-4">
          <p className="text-sm text-gray-500 dark:text-gray-400">Total Componentes</p>
      ...
      <Card extra="p-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <select
            value={sistemaFilter}
            onChange={(e) => { setSistemaFilter(e.target.value); setPage(0); }}
          ...
          <div className="flex h-10 items-center rounded-lg border border-gray-200 bg-white px-3 dark:border-navy-600 dark:bg-navy-700">
            ...
            <input
              ...
              placeholder="Buscar por codigo, componente, descripcion..."
              ...
              {componentes.map((comp) => (
                ...
                >
                  ...
                  <td className="hidden px-3 py-3 lg:table-cell">
                    <span
                      className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${
                        SISTEMA_COLORS[comp.sistema] || 'bg-gray-100 text-gray-600'
                      }`}
                    >
                      {comp.sistema || '—'}
                    </span>
                  ...
                  <td className="hidden px-3 py-3 md:table-cell">
                    <span className="text-sm text-navy-700 dark:text-gray-300">{comp.ubicacion}</span>
                  </td>
                  <td className="hidden px-3 py-3 md:table-cell">
                    <span className="text-sm text-navy-700 dark:text-gray-300">
                      {comp.marca}{comp.modelo ? ` / ${comp.modelo}` : ''}
                    </span>
                  </td>
                  <td className="hidden px-3 py-3 text-center sm:table-cell">
                    <span
                      className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${
                        CRITICIDAD_INFRA_COLORS[comp.criticidad] || 'bg-gray-100 text-gray-600'
                      }`}
                    >
                      {CRITICIDAD_INFRA_LABELS[comp.criticidad] || comp.criticidad}
                    </span>
                  </td>
                  <td className="px-3 py-3 text-center">
                    <span
                      className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${
                        ESTADO_INFRA_COLORS[comp.estado] || 'bg-gray-100 text-gray-600'
                      }`}
                    >
                      {ESTADO_INFRA_LABELS[comp.estado] || comp.estado}
                    </span>
                  </td>
                  <td className="px-3 py-3 text-center">
                    {comp.convenioActivo ? (
                  ...
                  <td className="hidden px-3 py-3 text-center xl:table-cell">
                    <UltimoMttoBadge
                      fecha={comp.ultimaFechaMantenimiento}
                      estado={comp.ultimoEstadoMantenimiento}
                    />
                  </td>
                  <td className="hidden px-3 py-3 text-center xl:table-cell">
                    <CumplimientoBadge
                      estado={comp.estadoCumplimientoMantenimiento}
                      porcentaje={comp.cumplimientoPorcentaje}
                      periodosCumplidos={comp.periodosCumplidos}
                      periodosEsperados={comp.periodosEsperados}
                      periodosFaltantes={comp.periodosFaltantes}
                      estadoFisico={comp.estado}
                      fechaBaja={comp.fechaBaja}
                    />
                  </td>
                  <td className="hidden px-3 py-3 text-center 2xl:table-cell">
                    <span className="text-sm text-navy-700 dark:text-gray-300">
                      {comp.proximaInspeccion || '—'}
                    </span>
                  </td>
                  <td className="hidden px-3 py-3 text-center 2xl:table-cell">
                    <span className="text-sm text-navy-700 dark:text-gray-300">
                      {comp.frecuencia ? `${comp.frecuencia}m` : '—'}
                    </span>
                  </td>
      ...
      <InfraestructuraDetailModal
        ...
        onEdit={() => {
          setDetailModalOpen(false);
          if (viewingComponente) {
            setEditingComponente(viewingComponente);
            setFormModalOpen(true);
          }
        }}
        componente={viewingComponente}
        onActivoChanged={() => {
          fetchComponentes();
          fetchStatsGlobal();
        }}
