...
import InventarioIndustrialHistorialPanel from 'components/admin/inventario-industrial/InventarioIndustrialHistorialPanel';
import InventarioIndustrialArchivosPanel from 'components/admin/inventario-industrial/InventarioIndustrialArchivosPanel';
...
export default function InventarioIndustrialDetailModal({ isOpen, onClose, onEdit, equipo, onActivoChanged }: Props) {
      ...
      <div className="relative max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl dark:bg-navy-800">
          ...
          <>
            <div className="mb-4 rounded-xl bg-gray-50 p-4 dark:bg-navy-900">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <span
                  className={`inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${
                    CRITICIDAD_COLORS[equipo.criticidad] || 'bg-gray-100 text-gray-600'
                  }`}
                >
                  Criticidad: {CRITICIDAD_LABELS[equipo.criticidad] || equipo.criticidad}
                </span>
                {equipo.requiereAutorizacion && (
                  <span className="inline-block rounded-full bg-indigo-100 px-2.5 py-1 text-xs font-semibold text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <DetailRow label="Ubicacion" value={equipo.ubicacion} />
              <DetailRow label="Tipo de Equipo" value={equipo.tipoEquipo} />
              <DetailRow label="Serie" value={equipo.serie} />
              <DetailRow label="Inventario" value={equipo.inventario} />
              <DetailRow label="Capacidad / Potencia" value={equipo.capacidad} />
              <DetailRow label="Combustible / Energia" value={equipo.combustible} />
              <DetailRow label="Fecha de Instalacion" value={formatDate(equipo.fechaInstalacion)} />
        ...
        {activeTab === 'mantenimientos' && (
          <ActivoMantenimientosPanel
            activoId={equipo.id}
            activoClase="InventarioEquipoIndustrial"
            dominio="equipoIndustrial"
            fechaBase={equipo.fechaInstalacion}
        ...
        {/* Tab: Historial */}
        {activeTab === 'historial' && (
          <InventarioIndustrialHistorialPanel equipoId={equipo.id} />
        )}

        {/* Tab: Archivos */}
        {activeTab === 'archivos' && (
          <InventarioIndustrialArchivosPanel equipoId={equipo.id} />
        )}
