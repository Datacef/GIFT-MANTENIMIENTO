...
import { MdUpload, MdDelete, MdInsertDriveFile, MdOpenInNew, MdFilterList } from 'react-icons/md';
import Parse from 'utils/parseClient';
import Swal from 'sweetalert2';
import {
  ArchivoAdjuntoIndustrial,
  ARCHIVO_CATEGORIA_INDUSTRIAL_OPTIONS,
  ARCHIVO_CATEGORIA_INDUSTRIAL_LABELS,
  ARCHIVO_CATEGORIA_INDUSTRIAL_COLORS,
...
export default function InventarioIndustrialArchivosPanel({ equipoId }: Props) {
  const [archivos, setArchivos] = useState<ArchivoAdjuntoIndustrial[]>([]);
  ...
  const fetchArchivos = useCallback(async () => {
    setLoading(true);
    try {
      const result = await InventarioIndustrialService.getArchivos(equipoId);
      setArchivos(result);
    } catch (error) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await InventarioIndustrialService.adjuntarArchivo(equipoId, file, selectedCategoria);
      setUploadProgress(100);

      const catLabel = ARCHIVO_CATEGORIA_INDUSTRIAL_LABELS[selectedCategoria] || selectedCategoria;
      Swal.fire({
        icon: 'success',
  ...
  const handleDelete = async (archivo: ArchivoAdjuntoIndustrial) => {
    ...
    try {
      await InventarioIndustrialService.eliminarArchivo(equipoId, archivo.nombre, archivo.url);
      Swal.fire({
        icon: 'success',
      ...
      Swal.fire('Error', error.message || 'Error al eliminar el archivo', 'error');
    }
  };
    ...
    <div>
        ...
        <div className="mb-4 space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              >
                {ARCHIVO_CATEGORIA_INDUSTRIAL_OPTIONS.map((opt) => (
      ...
      {archivos.length > 0 && (
        <div className="mb-3 flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {ARCHIVO_CATEGORIA_INDUSTRIAL_OPTIONS.map((opt) => {
              const count = archivos.filter((a) => (a.categoria || 'otro') === opt.value).length;
      ...
      {archivosFiltrados.length === 0 ? (
        <div className="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
          {archivos.length === 0
            ? 'No hay archivos adjuntos para este equipo.'
        ...
        <div className="space-y-2">
          {archivosFiltrados.map((archivo, index) => {
            ...
            const catLabel = ARCHIVO_CATEGORIA_INDUSTRIAL_LABELS[cat] || cat;
            const catColor = ARCHIVO_CATEGORIA_INDUSTRIAL_COLORS[cat] || ARCHIVO_CATEGORIA_INDUSTRIAL_COLORS['otro'];
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <MdInsertDriveFile className={`h-8 w-8 flex-shrink-0 ${getFileIcon(archivo.tipo)}`} />
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${catColor}`}>
                        {catLabel}
                      </span>
                    </div>
                    <p className="mt-0.5 text-xs text-gray-400">
                      Subido por {archivo.subidoPor} el {formatDate(archivo.fecha)}
                    </p>
                  </div>
                </div>
