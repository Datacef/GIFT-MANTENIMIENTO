...
import { HistorialIndustrialEntry } from 'types/inventario-industrial.types';
...
export default function InventarioIndustrialHistorialPanel({ equipoId }: Props) {
  const [historial, setHistorial] = useState<HistorialIndustrialEntry[]>([]);
  ...
  const fetchHistorial = useCallback(async () => {
    setLoading(true);
    try {
      const result = await InventarioIndustrialService.getHistorial(equipoId, pageSize, skip);
      if (skip === 0) {
