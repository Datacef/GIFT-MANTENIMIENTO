...
import {
  ArchivoAdjunto,
  ARCHIVO_CATEGORIA_OPTIONS,
  ARCHIVO_CATEGORIA_LABELS,
  ARCHIVO_CATEGORIA_COLORS,
...
export default function InventarioArchivosPanel({ equipoId }: Props) {
  ...
  const fetchArchivos = useCallback(async () => {
    setLoading(true);
    try {
      const result = await InventarioEquipoService.getArchivos(equipoId);
      setArchivos(result);
    } catch (error) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await InventarioEquipoService.adjuntarArchivo(equipoId, file, selectedCategoria);
      setUploadProgress(100);

      const catLabel = ARCHIVO_CATEGORIA_LABELS[selectedCategoria] || selectedCategoria;
      Swal.fire({
        icon: 'success',
  ...
  const handleDelete = async (archivo: ArchivoAdjunto) => {
    ...
    try {
      await InventarioEquipoService.eliminarArchivo(equipoId, archivo.nombre, archivo.url);
      Swal.fire({
        icon: 'success',
        ...
        <div className="mb-4 space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              >
                {ARCHIVO_CATEGORIA_OPTIONS.map((opt) => (
      ...
      {archivosFiltrados.length === 0 ? (
        ...
        <div className="space-y-2">
          {archivosFiltrados.map((archivo, index) => {
            ...
            const catLabel = ARCHIVO_CATEGORIA_LABELS[cat] || cat;
            const catColor = ARCHIVO_CATEGORIA_COLORS[cat] || ARCHIVO_CATEGORIA_COLORS['otro'];
