...
import { HistorialEntry } from 'types/inventario-equipo.types';
...
export default function InventarioHistorialPanel({ equipoId }: Props) {
  ...
  const fetchHistorial = useCallback(async () => {
    setLoading(true);
    try {
      const result = await InventarioEquipoService.getHistorial(equipoId, pageSize, skip);
      if (skip === 0) {
