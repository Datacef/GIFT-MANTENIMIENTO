...
import {
  ArchivoAdjuntoInfra,
  ARCHIVO_CATEGORIA_INFRA_OPTIONS,
  ARCHIVO_CATEGORIA_INFRA_LABELS,
  ARCHIVO_CATEGORIA_INFRA_COLORS,
...
export default function InfraestructuraArchivosPanel({ componenteId }: Props) {
  const [archivos, setArchivos] = useState<ArchivoAdjuntoInfra[]>([]);
  ...
  const fetchArchivos = useCallback(async () => {
    setLoading(true);
    try {
      const result = await InventarioInfraestructuraService.getArchivos(componenteId);
      setArchivos(result);
    } catch (error) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await InventarioInfraestructuraService.adjuntarArchivo(componenteId, file, selectedCategoria);
      setUploadProgress(100);

      const catLabel = ARCHIVO_CATEGORIA_INFRA_LABELS[selectedCategoria] || selectedCategoria;
      Swal.fire({
        icon: 'success',
  ...
  const handleDelete = async (archivo: ArchivoAdjuntoInfra) => {
    ...
    try {
      await InventarioInfraestructuraService.eliminarArchivo(componenteId, archivo.nombre, archivo.url);
      Swal.fire({
        icon: 'success',
    ...
    <div>
        ...
        <div className="mb-4 space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              >
                {ARCHIVO_CATEGORIA_INFRA_OPTIONS.map((opt) => (
      ...
      {archivosFiltrados.length === 0 ? (
        <div className="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
          {archivos.length === 0
            ? 'No hay archivos adjuntos para este componente.'
        ...
        <div className="space-y-2">
          {archivosFiltrados.map((archivo, index) => {
            ...
            const catLabel = ARCHIVO_CATEGORIA_INFRA_LABELS[cat] || cat;
            const catColor = ARCHIVO_CATEGORIA_INFRA_COLORS[cat] || ARCHIVO_CATEGORIA_INFRA_COLORS['otro'];
