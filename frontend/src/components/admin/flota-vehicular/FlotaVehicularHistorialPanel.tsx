...
import { HistorialFlotaEntry } from 'types/inventario-flota.types';
...
export default function FlotaVehicularHistorialPanel({ vehiculoId }: Props) {
  const [historial, setHistorial] = useState<HistorialFlotaEntry[]>([]);
  ...
  const fetchHistorial = useCallback(async () => {
    setLoading(true);
    try {
      const result = await InventarioFlotaService.getHistorial(vehiculoId, pageSize, skip);
      if (skip === 0) {
  ...
  if (historial.length === 0) {
    return (
      <div className="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
        No hay registros de historial para este vehiculo.
      </div>
    );
  }
