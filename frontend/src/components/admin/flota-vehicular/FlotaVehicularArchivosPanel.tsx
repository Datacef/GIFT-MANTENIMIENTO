...
import {
  ArchivoAdjuntoFlota,
  ARCHIVO_CATEGORIA_FLOTA_OPTIONS,
  ARCHIVO_CATEGORIA_FLOTA_LABELS,
  ARCHIVO_CATEGORIA_FLOTA_COLORS,
...
export default function FlotaVehicularArchivosPanel({ vehiculoId }: Props) {
  const [archivos, setArchivos] = useState<ArchivoAdjuntoFlota[]>([]);
  ...
  const fetchArchivos = useCallback(async () => {
    setLoading(true);
    try {
      const result = await InventarioFlotaService.getArchivos(vehiculoId);
      setArchivos(result);
    } catch (error) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await InventarioFlotaService.adjuntarArchivo(vehiculoId, file, selectedCategoria);
      setUploadProgress(100);

      const catLabel = ARCHIVO_CATEGORIA_FLOTA_LABELS[selectedCategoria] || selectedCategoria;
      Swal.fire({
        icon: 'success',
  ...
  const handleDelete = async (archivo: ArchivoAdjuntoFlota) => {
    ...
    try {
      await InventarioFlotaService.eliminarArchivo(vehiculoId, archivo.nombre, archivo.url);
      Swal.fire({
        icon: 'success',
    ...
    <div>
        ...
        <div className="mb-4 space-y-3">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              >
                {ARCHIVO_CATEGORIA_FLOTA_OPTIONS.map((opt) => (
      ...
      {archivosFiltrados.length === 0 ? (
        <div className="py-8 text-center text-sm text-gray-500 dark:text-gray-400">
          {archivos.length === 0
            ? 'No hay archivos adjuntos para este vehiculo.'
        ...
        <div className="space-y-2">
          {archivosFiltrados.map((archivo, index) => {
            ...
            const catLabel = ARCHIVO_CATEGORIA_FLOTA_LABELS[cat] || cat;
            const catColor = ARCHIVO_CATEGORIA_FLOTA_COLORS[cat] || ARCHIVO_CATEGORIA_FLOTA_COLORS['otro'];
