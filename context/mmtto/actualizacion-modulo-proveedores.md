...
## Fix — Extensiones de Contrato (2026-04-12)

### Bug corregido
El botón **Agregar Extensión** en el modal de detalle de licitación no respondía visualmente: el `ExtensionFormModal` sí se abría, pero quedaba oculto detrás del `LicitacionDetailModal` por conflicto de z-index (ambos usaban `z-50` / `z-[60]`).

**Arreglo:** `ExtensionFormModal` ahora usa `z-[70]` para quedar por encima del modal padre.

### Mejoras añadidas al modal
- Muestra **fecha término original** (solo lectura, no se modifica nunca) y **fecha término efectiva actual** como referencia.
- Nota explícita al usuario: *"La nueva fecha actualizará el convenio en los 4 inventarios (Médicos, Industriales, Flota, Infraestructura)"*.
- Validación: la nueva fecha debe ser **posterior** a la fecha término efectiva actual.

### Comportamiento confirmado (ya funcionaba correctamente en backend)

1. **Fecha original intacta**: el campo `fechaTermino` de `Licitacion` nunca se modifica. Las extensiones se acumulan en el array `extensiones`.
2. **Fecha efectiva calculada**: `calcularEstadoLicitacion()` itera las extensiones y devuelve la última fecha.
3. **Sincronización con los 4 inventarios**: tras guardar la extensión, `sincronizarConveniosParaTipo(inventarioTipo)` actualiza `convenioActivo`, `proveedorRut`, `proveedorNombre`, `numeroLicitacion` y `fechaTerminoConvenio` en los equipos afectados de:
...
4. **Historial**: cada extensión queda registrada en `LicitacionHistorial` con acción `extension`.
